...
class ResumeScorer:
    def __init__(self, job: Job):
        ...
        self.job_kw_embeddings = np.asarray(
            [self.fb_model.wv[kw] for kw in self.job.keywords]
        )

    def closest_keyword_index(self, sample: str) -> str:
        """Takes the keyword list as embeddings form a job, and a sample.
        Finds the closest keyword
    ...
    def get_evaluation_text_for_df_row(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df.loc[
            df["type"].isin(["summary", "patents", "interests"]), ["scoring_text"]
        ] = df["value"]

        df.loc[df["type"].isin(["contactInfo"]), ["scoring_text"]] = ""
    ...
    def score_resume_as_dataframe(self, resume: pd.DataFrame) -> pd.DataFrame:
        ...
        resume["score"] = resume.apply(
            lambda x: self.score_text(x["scoring_text"]) * x["multiplier"], axis=1
        )
    ...
    def shorten_resume(self, resume: pd.DataFrame) -> pd.DataFrame:
        resume_part_heuristics = {
            "experience": 1.0,
        }
        res = resume.copy()
        res["text_len"] = len(res["scoring_text"])



        cutoff = 5
        return resume.sort_values("label")[:-cutoff]
